...
# Step 1: Get Nevada shapefile and set projection
nv_shape <- states(cb = TRUE) |> 
  st_as_sf() |> 
  filter(NAME == "Nevada") |> 
  st_transform(crs = "+proj=lcc +lat_1=33 +lat_2=45 +lon_0=-116 +datum=WGS84 +units=m +no_defs")

# Step 2: Fetch elevation data for Nevada
dem <- get_elev_raster(
  locations = nv_shape,
  prj = st_crs(nv_shape)$proj4string,
  z = 6,
  src = "aws"
)

# Step 3: Mask and crop DEM to Nevada's boundary
dem <- rast(dem) |> 
  mask(nv_shape) |> 
  crop(nv_shape)
...
writePNG(im, target = "Nevada_ShadedRelief.png")

# Step 8: Enhance the map using magick
im1 <- image_read("Nevada_ShadedRelief.png") |> 
  image_trim() |> 
...
im2 <- im1 |> 
  image_annotate("Nevada", font = "sans", color = "#01611F", weight = 700, 
...
image_write(im2, path = "Nevada_ShadedRelief_Final.png", format = "png", quality = 95)
